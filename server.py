...
@flask_app.route('/localization.js')
def serve_localization():
    """Serve the localization JavaScript file"""
    try:
        return send_file('localization.js', mimetype='application/javascript')
    except FileNotFoundError:
        return "Localization file not found", 404

def start_flask_server():
    """Start Flask server in separate thread"""
    flask_app.run(host='127.0.0.1', port=9767, debug=False, use_reloader=False)
...
# --- ОБЩАЯ ЛОГИКА ДЛЯ РАСЧЕТА SL/TP ---
    ...
    if sl_value and sl_unit:
        ...
        # Специальная логика для криптовалют
        if "BTC" in mt5_symbol or "ETH" in mt5_symbol or "LTC" in mt5_symbol:
            if sl_unit == 'dollar':
                # Для крипты: прямой расчет - $1 убытка = движение цены
                price_change = sl_value / volume
                if action == "buy":
                    sl_price = current_price - price_change
                else:
                    sl_price = current_price + price_change
            else:  # pips/points - для крипты это доллары
                price_change = sl_value
                if action == "buy":
                    sl_price = current_price - price_change
                else:
                    sl_price = current_price + price_change
        # Специальная логика для золота
        elif "XAU" in mt5_symbol or "GOLD" in mt5_symbol:
            if sl_unit == 'dollar':
                # Для золота: $1 прибыли при 0.01 лоте = $1 движения цены
    ...
    if tp_value and tp_unit:
        ...
        if "BTC" in mt5_symbol or "ETH" in mt5_symbol or "LTC" in mt5_symbol:
            if tp_unit == 'dollar':
                price_change = tp_value / volume
                if action == "buy":
                    tp_price = current_price + price_change
                else:
                    tp_price = current_price - price_change
            else:  # pips/points
                price_change = tp_value
                if action == "buy":
                    tp_price = current_price + price_change
                else:
                    tp_price = current_price - price_change
    ...
    # Сохраняем количество позиций ДО открытия новой
    positions_before = mt5.positions_get()
    positions_count_before = len(positions_before) if positions_before else 0
    ...
    if result and result.retcode == mt5.TRADE_RETCODE_DONE:
        ...
        if order_mode in ["buy_limit", "sell_limit"]:
            ...
            # ИСПРАВЛЕНИЕ ДЛЯ РЫНОЧНЫХ ОРДЕРОВ
            if has_automation:
                # Ждем немного, чтобы позиция появилась в системе
                await asyncio.sleep(0.5)
                
                # Получаем обновленный список позиций
                positions_after = mt5.positions_get()
                
                if positions_after and len(positions_after) > positions_count_before:
                    # Находим новую позицию - она должна быть последней открытой
                    # и соответствовать нашему символу и объему
                    for pos in positions_after:
                        # Проверяем, что это новая позиция
                        is_new = True
                        if positions_before:
                            for old_pos in positions_before:
                                if pos.ticket == old_pos.ticket:
                                    is_new = False
                                    break
                        
                        # Если это новая позиция с нашими параметрами
                        if is_new and pos.symbol == mt5_symbol and pos.volume == volume:
                            position_monitors[pos.ticket] = automation_settings
                            logger.info(f"✓ Automation settings applied to position #{pos.ticket}")
                            logger.info(f"  Trailing: {automation_settings['trailing']}, Distance: ${automation_settings['trailing_distance']}")
                            logger.info(f"  Breakeven: {automation_settings['breakeven']}, Profit: ${automation_settings['breakeven_profit']}")
                            break
                else:
                    # Если не нашли новую позицию, сохраняем временно с deal ID
                    # для подхвата в send_positions
                    position_id = result.deal if hasattr(result, 'deal') else result.order
                    position_monitors[position_id] = automation_settings
                    logger.warning(f"Position not found yet, saved with deal ID #{position_id}")
...
async def modify_position(websocket, data):
    ...
    # ИСПРАВЛЕНИЕ: Обрабатываем случай когда передается 0 или null
    # Если передан 0 или None, явно устанавливаем 0.0
    if new_sl_price == 0 or new_sl_price == "0":
        new_sl_price = 0.0
    elif new_sl_price is not None:
        new_sl_price = float(new_sl_price)
    
    if new_tp_price == 0 or new_tp_price == "0":
        new_tp_price = 0.0
    elif new_tp_price is not None:
    ...
    # ИСПРАВЛЕНИЕ: Определяем финальные значения SL и TP
    # Если новое значение передано - используем его, иначе берем текущее из позиции
    final_sl = 0.0
    final_tp = 0.0
    ...
    if new_sl_price is not None:
        ...
        # Если у позиции есть SL - сохраняем его
        final_sl = float(position.sl) if position.sl > 0 else 0.0
    
    if new_tp_price is not None:
        final_tp = float(new_tp_price)
    else:
        # Если у позиции есть TP - сохраняем его
        final_tp = float(position.tp) if position.tp > 0 else 0.0
    
    # Создаем запрос с правильными значениями
    request = {
    ...
    if symbol_info:
        request["sl"] = round(request["sl"], symbol_info.digits) if request["sl"] > 0 else 0.0
        request["tp"] = round(request["tp"], symbol_info.digits) if request["tp"] > 0 else 0.0
    
    # Логируем для отладки
    logger.info(f"Modifying position #{position_id}: SL={request['sl']}, TP={request['tp']}")
    ...
    if result and result.retcode == mt5.TRADE_RETCODE_DONE:
        ...
        logger.error(f"{error_msg}, retcode: {result.retcode if result else 'None'}")
        await send_error(websocket, error_msg)
...
async def main():
    ...
    print("Login Interface running at: http://127.0.0.1:9767")
